...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // If we scroll down more than 20 pixels, trigger the full-width state
  ...
  return (
    <>
      ...
      >
        ...
        {/* Dynamic Container: Pill when at top, Full Width when scrolled */}
        <div 
        ...
        >
          ...
          <div className="flex-1 flex justify-start items-center min-w-max">
            <img 
              ...
              className={`w-auto object-contain cursor-pointer transition-all duration-500 ${
                isScrolled ? 'h-10 sm:h-12' : 'h-12 sm:h-14' // ENLARGED LOGO SIZES
              }`} 
            />
          </div>
          ...
          {/* ==============================
              ...
              ============================== */}
          {/* TIGHTENED WHITESPACE: changed space-x-7 to space-x-4 xl:space-x-5 */}
          ...
          <div className="hidden xl:flex flex-1 justify-end items-center gap-6 min-w-max">
            {/* Bright Yellow Phone Pill */}
            <div className="hidden xl:flex flex-[2] justify-center items-center space-x-4 xl:space-x-5">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-[14px] font-bold text-[#1e293b] hover:text-[#eab308] transition-colors duration-200 tracking-wide whitespace-nowrap"
              >
                {link.name}
              </a>
            ))}
            ...
            <div className="relative group">
              <button className="flex items-center text-[14px] font-bold text-[#1e293b] hover:text-[#eab308] transition-colors duration-200 tracking-wide whitespace-nowrap">
                MORE
                {/* Yellow chevron */}
                <svg className="ml-1 w-4 h-4 text-[#eab308] transition-transform duration-200 group-hover:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 9l-7 7-7-7" /></svg>
              </button>
              ...
              <div className="absolute left-1/2 -translate-x-1/2 mt-4 w-48 bg-white border border-gray-100 rounded-2xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 translate-y-2 group-hover:translate-y-0 overflow-hidden">
                  <a href="#" className="block px-4 py-3 text-sm font-bold text-[#1e293b] hover:bg-gray-50 hover:text-[#eab308]">Contact Us</a>
              </div>
            </div>
          </div>
            <a 
          ...
          {/* ==============================
              MOBILE MENU BUTTON (Visible when smaller than xl)
              ============================== */}
          ...
          <div className="bg-white/95 backdrop-blur-xl shadow-2xl rounded-3xl p-5 flex flex-col space-y-2 border border-gray-100 mx-4">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="px-4 py-3 rounded-xl text-sm font-bold text-[#1e293b] hover:bg-gray-100 hover:text-[#eab308] transition-colors"
              >
                {link.name}
              </a>
            ))}
            ...
            <div>
              <button 
                ...
                className="w-full flex justify-between items-center px-4 py-3 rounded-xl text-sm font-bold text-[#1e293b] hover:bg-gray-100 hover:text-[#eab308] transition-colors"
              >
                MORE
                <svg className={`w-4 h-4 text-[#eab308] transition-transform duration-200 ${isMobileMoreOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 9l-7 7-7-7" /></svg>
              </button>
              <div className={`pl-4 overflow-hidden transition-all duration-200 ${isMobileMoreOpen ? 'max-h-40 opacity-100 mt-1' : 'max-h-0 opacity-0'}`}>
                  <a href="#" className="block px-4 py-2 text-sm font-bold text-gray-500 hover:text-[#eab308]">Contact Us</a>
              </div>
            </div>
            ...
            <div className="mt-4 pt-5 border-t border-gray-200 flex flex-col gap-3">
              <a 
                href="tel:+13158981412" 
                className="flex justify-center items-center gap-2 w-full bg-[#facc15] text-[#1e293b] px-5 py-3.5 rounded-xl font-bold text-[15px] hover:bg-[#eab308] transition-colors shadow-sm"
              >
                +1 315 898 1412
              </a>
            </div>
